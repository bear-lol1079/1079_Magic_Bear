...
/* Bounds for 1:1 optimizer only (kept as before) */
...
function ensureMinInf(march, minInf, stockLeftover){
  ...
  if (need > 0) {
    const fromArc = Math.min(march.arc, need);
    march.arc -= fromArc;
    march.inf += fromArc;
    need -= fromArc;
  }
...
function replaceExcessInfWithPriority(march, minInf, stockLeftover){
  ...
  if (toCav > 0) {
    march.cav += toCav;
    march.inf -= toCav;
    stockLeftover.cav -= toCav;
    stockLeftover.inf += toCav;
    excess -= toCav;
  }
...
function adjustCallRallyCapsAndBias(rally, leftover, rallySize, opts = {}) {
  ...
  if (rally.inf < minInf) {
    ...
    if (need > 0) {
      ...
      rally.arc -= fromArc; rally.inf += fromArc; need -= fromArc;
    }
  }
  ...
  if (want > 0 && leftover.arc > 0) {
    ...
    if (rally.inf > minInf) {
      ...
      rally.inf -= takeFromInf;
      rally.arc += takeFromInf;
      leftover.arc -= takeFromInf;
    ...
    if (want > 0 && rally.cav > 0 && leftover.arc > 0) {
      ...
      rally.arc += takeFromCav;
      leftover.arc -= takeFromCav;
      leftover.cav += takeFromCav;
      want -= takeFromCav;
    }
  }
  ...
  if (rally.inf < minInf) {
    ...
    const fromArc = Math.min(rally.arc, need);
    rally.arc -= fromArc; rally.inf += fromArc;

    let still = need - fromArc;
    if (still > 0) {
      const fromCav = Math.min(rally.cav, still);
...
function deriveFractionsFromCompositions(rally, joins){
  ...
  const T = Math.max(1, I + C + A);
  return { i: I/T, c: C/T, a: A/T };
}
